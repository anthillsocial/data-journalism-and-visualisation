...
function setup() {// Variable to save air data in
  ...
  // FILTER DATA
  dataset = filterTopNItems(dataset, column=2, items = 10); // Grab 10 highest values in col 2
  dataset = filterTopNItems(dataset, column=0, items = 10); // Order the dataset by date
...
// Sort the dataset by a column and return top 'N' items
function filterTopNItems(mydataset, column, items) {
    // Create an array filled with a very low number '-10000000'
    let topItems = []
    for (let row = 0; row < items; row++) {
      topItems[row] = [row, -10000000]
    }
    // Loop through the dataset 
    for (let row = 0; row < mydataset.getRowCount(); row++) {
      newValue = mydataset.getNum(row, column); // Get value
      topItems.sort(
        function sortFunction(a, b) {
        if (a[1]===b[1]){return 0;}else{return(a[1]<b[1])?-1:1;}
      });        // Sort the top items
      lowestValue = topItems[0][1]
      if (newValue > lowestValue) {
          topItems[0] = [row, newValue]
      }
    }
    // Create a new table with the filtered rows
    newDataset = new p5.Table()
    // Add the filtered rows to the new table
    for (let row = 0; row < topItems.length; row++) {
      console.log(`${row}: ${topItems[row][0]} ${topItems[row][1]}`)
      saveRow = mydataset.getRow(Number(topItems[row][0])); // Get value
      console.log(saveRow)
      newDataset.addRow(saveRow)
    }
    return newDataset;
}
